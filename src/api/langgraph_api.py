...
async def create_knowledge_base(request: KnowledgeBaseRequest):
    """创建知识库"""
    from .routes import create_knowledge_base as create_kb_route
    return await create_kb_route(request)


async def list_knowledge_bases():
    """列出知识库"""
    from .routes import list_knowledge_bases as list_kb_route
    return await list_kb_route()


async def search_knowledge_base(kb_name: str, query: str, k: int = 5):
    """搜索知识库"""
    from .routes import search_knowledge_base as search_kb_route
    return await search_kb_route(kb_name, query, k)


async def list_tools():
    """列出工具"""
    from .routes import list_tools as list_tools_route
    return await list_tools_route()


async def call_tool(tool_name: str, arguments: dict):
    """调用工具"""
    from .routes import call_tool as call_tool_route
